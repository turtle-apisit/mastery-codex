...
0b. **Right after every Simulation set Vega drafts, before a single row is inserted** — like step 0, this is never sampled and has no weekly cadence. It is two checks, and conflating them is the usual way a bad set gets through:
   - **Conformance** — the nine mechanical points in `SIMULATION.md` §8. Is it exactly 10 `choice` + 2 `written`; is every item bound to a `technique_id` in the set's own subject; does any question repeat one already asked of that Technique in any earlier set; where a topic repeats, did the cognitive level or the `source_basis` actually change; are the correct answers spread across all four letters; does every distractor name a specific misunderstanding; does every rubric define what a `content_score` of 3 requires; is `source_basis` accurate on each item. These are checkable without judgment, and a set failing any of them is not the thing that was designed, however good its questions read.
   - **Source alignment** — the same test as step 1, applied per item rather than per file: open the cited source and confirm the item tests what is actually taught there, at a depth the source supports. An `outside` item is held to a different bar, not a lower one: it may go past the lecture, but it may not contradict it.
   State a verdict per item and per check. Nova cross-checks the same set against the same sources independently, and only once both agree does Nova insert the items and the `exam_set_reviews` row that records the gate having run.
...
3. Classify each finding:
   ...
   - **Simulation set failure** (step 0b, either check) → report to Vega with the failing item and the point it fails; don't rewrite the item and don't drop it to make the count work. A twelve-item round with one bad item is an eleven-item round plus a repair, and Vega owns the repair.
